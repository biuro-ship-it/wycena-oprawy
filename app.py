...
st.set_page_config(page_title="EuroRama Ekspert v10.2", page_icon="🖼️", layout="wide")
...
# --- 4. POPRAWIONY EKSPORT PDF (FIX: bytearray to bytes) ---
def create_pdf_bytes(d: Dict) -> bytes:
    ...
    if d.get('addons'): pdf.cell(0, 10, f"- Dodatki: {d['addons']}", ln=True)
    ...
    # Konwersja na ASCII dla bezpieczeństwa PDF
    uwagi = str(d.get('uwagi', '')).encode('ascii', 'ignore').decode('ascii')
    pdf.multi_cell(0, 10, f"Uwagi: {uwagi}")
    
    # KLUCZOWA POPRAWKA: Jawna konwersja na bajty
...
if not db:
    st.info("Proszę wgrać bazę cennik.csv.")
else:
    with st.container():
        c1, c2 = st.columns([2, 1])
        codes = sorted(db)
        sel_code = c1.selectbox("Wybierz kod listwy:", options=codes)
    ...
    with st.expander("📝 Dodatki i Uwagi"):
        ...
        u_notes = st.text_area("Uwagi klienta")
    ...
    if st.button("🚀 WYCEŃ", type="primary", use_container_width=True):
        ...
        mb, m2, p_l, p_r, f_l, f_r = calculate_prices(db[sel_code], in_w, in_h, def_m_l, def_m_r, ex_man, ex_man, mat_netto * m2_pow, c_min)
        
        st.session_state.calc_results = {
            'kod': sel_code.upper(), 's': in_w, 'w': in_h, 'mb': mb, 'm2': m2,
            'prod_l': p_l, 'prod_r': p_r, 'f_l': f_l, 'f_r': f_r, 'notes': u_notes, 'date': w_date,
    ...
    if st.session_state.calc_results:
        ...
        st.divider()
        st.markdown(f"📊 {res['mb']:.2f} mb | {res['s']}x{res['w']} cm")
        ...
        if st.session_state.selected_option:
            ...
            with ex1:
                pdf_data = {'data':res['date'], 'kod':res['kod'], 'format':f"{res['s']}x{res['w']}", 'opcja':opt_name, 'cena':opt_price, 'uwagi':res['notes'], 'addons':res['addons']}
                try:
...
if st.session_state.history:
    st.write("---")
    st.subheader("🕒 Ostatnie wyceny")
    for h in st.session_state.history[:20]:
        st.markdown(f"<div style='background:#252525; padding:10px; border-radius:8px; margin-bottom:5px; border-left:4px solid #0e63d1'>{h}</div>", unsafe_allow_html=True)

st.markdown(f'''<div class="footer"><p>📞 Zadzwoń: <b>15 876 30 16</b></p><p><a href="https://godek.eu/upload/instrukcja.pdf" target="_blank" style="color:#0e63d1">📖 Instrukcja Obsługi (PDF)</a></p></div>''', unsafe_allow_html=True)
